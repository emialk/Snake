...
int main(void) {
  InitWindow(glob::winWidth, glob::winHeight, glob::winTitle.data());
  SetTargetFPS(60);

  world_grid::WorldGrid world{glob::gridWidth, glob::gridHeight,
                              glob::gridCntVert, glob::gridCntHorz};

  player::Player snake{{.x = floorf(glob::gridCntHorz / 2.0),
                        .y = floorf(glob::gridCntVert / 2.0)}};
  ...
  while (!WindowShouldClose()) {
    ...
    Image img =
        GenImageChecked(glob::winWidth, glob::winHeight, glob::gridWidth,
                        glob::gridHeight, DARKBROWN, DARKGRAY);
    ...
    for (int row = 0; row < glob::gridCntVert; ++row) {
      for (int col = 0; col < glob::gridCntHorz; ++col) {
        ...
        if (cell == glob::ObjCode::SNAKE_HEAD) {
          ImageDrawRectangle(&img, col * glob::gridWidth,
                             row * glob::gridHeight, glob::gridWidth,
                             glob::gridHeight, BLACK);
          ImageDrawRectangle(&img, col * glob::gridWidth + 1,
                             row * glob::gridHeight + 1, glob::gridWidth - 2,
                             glob::gridHeight - 2, GREEN);
        }

        if (cell == glob::ObjCode::SNAKE_TAIL) {
          ImageDrawRectangle(&img, col * glob::gridWidth,
                             row * glob::gridHeight, glob::gridWidth,
                             glob::gridHeight, DARKGREEN);
        }
    ...
    DrawTextureRec(backgroundTexture,
                   (Rectangle){0, 0, glob::winWidth, glob::winHeight},
                   Vector2Zero(), WHITE);
